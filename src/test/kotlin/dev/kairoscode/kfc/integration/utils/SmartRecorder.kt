 ...
 * ```kotlin
 * // 대용량 데이터도 안전하게 레코딩
 * val allEtfs = client.etf.getList()
...
private fun recordSmartlyExample() {
    // 예시: ETF 목록 레코딩
    // val etfList: List<Etf> = client.etf.getList()
    // SmartRecorder.recordSmartly(
    //     data = etfList,
    //     category = RecordingConfig.Paths.EtfList.ALL,
    //     fileName = "etf_list_all"
    // )
    // 결과:
    // - 500개인 경우 -> etf_list_all.json (전체)
    // - 50,000개인 경우 -> etf_list_all_limited.json (처음 10,000개)
    // - 200,000개인 경우 -> etf_list_all_sample.json (랜덤 1,000개)
}
