...
/**
 * 전체 ETF 일별 시세 조회 Integration Test
 *
 * getAllDailyPrices() 함수의 실제 API 호출 테스트 및 응답 레코딩
 */
class EtfDailyPricesSpec : IntegrationTestBase() {

    @Test
    @DisplayName("거래일에 전체 ETF 시세를 조회할 수 있다")
    fun testGetAllDailyPricesOnTradingDay() = integrationTest {
        // Given: 고정 거래일
        val date = TestSymbols.TRADING_DAY // 2024-11-25 (월요일)

        // When: 전체 ETF 시세 조회
        val dailyPrices = client.funds.getAllDailyPrices(date)
    ...
    fun testGetAllDailyPricesOnNonTradingDay() = integrationTest {
        // Given: 고정 비거래일 (토요일)
    ...
    fun testTopGainersAndLosers() = integrationTest {
    ...
    fun testTopVolumeEtfs() = integrationTest {
        // Given: 전체 ETF 일별 시세 (고정 거래일)
        val date = TestSymbols.TRADING_DAY
        val dailyPrices = client.funds.getAllDailyPrices(date)
