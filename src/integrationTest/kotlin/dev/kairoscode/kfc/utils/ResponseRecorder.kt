 ...
 *
 * // 대용량 리스트 스트리밍 레코딩
 * ResponseRecorder.recordListStreaming(
 *     data = largeDataList,
 *     category = "eodhd/eod",
 *     fileName = "historical_prices",
 *     chunkSize = 1000
 * )
 * ```
 */
object ResponseRecorder {
    ...
    /**
     * 대용량 리스트를 청크 단위로 처리하여 기존 파일에 추가 (Append 모드)
     *
     * 이미 존재하는 JSON 배열 파일에 새로운 데이터를 추가합니다.
     * 기존 파일을 읽어서 새 데이터와 병합한 후 다시 저장합니다.
     *
     * @param data 추가할 리스트
     * @param category API 카테고리 경로
     * @param fileName 파일명 (확장자 제외)
     * @param chunkSize 한 번에 처리할 청크 크기 (기본값: 1000)
     * @return 최종 저장된 총 항목 수 (기존 + 새로운 데이터)
     *
     * ## 주의사항
     * - 기존 파일이 없으면 새로 생성합니다.
     * - 기존 파일이 유효한 JSON 배열이 아니면 새로 생성합니다.
     */
    inline fun <reified T> recordListStreamingAppend(
        data: List<T>,
        category: String,
        fileName: String,
        chunkSize: Int = DEFAULT_CHUNK_SIZE
        ...
        if (data.isEmpty()) {
            println("[Recording] Warning: No data to append for $category/$fileName")
            return 0
        }
