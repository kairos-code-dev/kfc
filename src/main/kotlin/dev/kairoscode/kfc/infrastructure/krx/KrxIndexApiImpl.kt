        ...
        return output
            .map { raw ->
                ...
                IndexInfo(
                    ...
                    market = market,
                    baseDate =
                        raw.getStringOrNull("BAS_TM_CONTN")?.replace(".", "/")?.toKrxDate()?.takeIf {
                            it !=
                                LocalDate.MIN
                        },
                    announcementDate =
                        raw.getStringOrNull("ANNC_TM_CONTN")?.replace(".", "/")?.toKrxDate()?.takeIf {
                            it !=
                                LocalDate.MIN
                        },
        ...
        return output
            .map { raw ->
                IndexOhlcvSnapshot(
                    name = raw.getString("IDX_NM"),
                    open =
                        raw.getStringOrNull("OPNPRC_IDX")?.toKrxBigDecimal()
                            ?: raw.getString("TDD_OPNPRC").toKrxBigDecimal(),
                    high =
                        raw.getStringOrNull("HGPRC_IDX")?.toKrxBigDecimal()
                            ?: raw.getString("TDD_HGPRC").toKrxBigDecimal(),
                    low =
                        raw.getStringOrNull("LWPRC_IDX")?.toKrxBigDecimal()
                            ?: raw.getString("TDD_LWPRC").toKrxBigDecimal(),
                    close =
                        raw.getStringOrNull("CLSPRC_IDX")?.toKrxBigDecimal()
                            ?: raw.getString("TDD_CLSPRC").toKrxBigDecimal(),
                    volume = raw.getString("ACC_TRDVOL").toKrxLong(),
                    tradingValue = raw.getStringOrNull("ACC_TRDVAL")?.toKrxLong(),
                )
        ...
        return output
            .map { raw ->
                IndexFundamentalSnapshot(
                    ...
                    close =
                        raw.getStringOrNull("CLSPRC_IDX")?.toKrxBigDecimal()
                            ?: raw.getString("CLSPRC").toKrxBigDecimal(),
                    changeRate = raw.getStringOrNull("FLUC_RT")?.toKrxRate(),
