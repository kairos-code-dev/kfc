...
import dev.kairoscode.kfc.infrastructure.common.util.toDirection
...
import dev.kairoscode.kfc.infrastructure.common.util.toKrxDouble
...
import dev.kairoscode.kfc.infrastructure.krx.internal.KrxApiParams
import dev.kairoscode.kfc.infrastructure.krx.internal.checkForErrors
import dev.kairoscode.kfc.infrastructure.krx.internal.extractOutput
...
internal class KrxFundsApiImpl(
    ...
    companion object {
        ...
        // ================================
        // 전체 ETF 일자별 투자자별 거래
        private const val BLD_ETF_ALL_INVESTOR_TRADING_DAILY = "dbms/MDC/STAT/standard/MDCSTAT04801"

        // 전체 ETF 기간별 투자자별 거래
        private const val BLD_ETF_ALL_INVESTOR_TRADING_PERIOD = "dbms/MDC/STAT/standard/MDCSTAT04802"
    ...
    override suspend fun getEtfPortfolio(
        isin: String,
        date: LocalDate,
            ...
            .map { raw ->
                TrackingError(
                    ...
                    trackingMultiple =
                        raw
                            .getString(
                                KrxApiFields.TrackingPerformance.TRACKING_MULTIPLE,
                            ).toKrxBigDecimal(),
                    trackingErrorRate =
                        raw
                            .getString(
                                KrxApiFields.TrackingPerformance.TRACKING_ERROR_RATE,
                            ).toKrxDouble(),
                )
    ...
    override suspend fun getEtfInvestorTrading(
        isin: String,
        date: LocalDate,
    ...
    override suspend fun getEtfShortSelling(
        ...
        return output
            .mapNotNull { item ->
                ...
                val shortVol =
                    raw[KrxApiFields.Volume.SHORT_VOLUME]
                        ?.toString()
                        ?.replace(",", "")
                        ?.replace("-", "0")
                        ?.toLongOrNull()
                        ?: 0L
                val shortVal =
                    raw[KrxApiFields.Volume.SHORT_VALUE]
                        ?.toString()
                        ?.replace(",", "")
                        ?.replace("-", "0")
                        ?.toLongOrNull()
                ...
                ShortSelling(
                    tradeDate =
                        raw[KrxApiFields.DateTime.TRADE_DATE]?.toString()?.replace("/", "")?.toKrxDate()
                    ...
                    shortValue = shortVal,
                    totalVolume =
                        raw[KrxApiFields.Volume.ACCUMULATED]?.toString()?.replace(",", "")?.toLongOrNull() ?: 0L,
                    totalValue =
                        raw[KrxApiFields.Volume.TRADING_VALUE]?.toString()?.replace(",", "")?.toLongOrNull() ?: 0L,
    ...
    override suspend fun getEtfShortBalance(
        ...
        return output
            .mapNotNull { raw ->
                ShortBalance(
                    tradeDate =
                        raw[KrxApiFields.DateTime.REPORT_DATE]?.toString()?.replace("/", "")?.toKrxDate()
                            ?: return@mapNotNull null,
                    ticker = ticker,
                    name = "", // API 응답에 name 필드 없음
                    shortBalance =
                        raw[KrxApiFields.ShortSelling.BALANCE_SHARES]?.toString()?.replace(",", "")?.toLongOrNull()
                            ?: 0L,
                    shortBalanceValue =
                        raw[KrxApiFields.ShortSelling.BALANCE_VALUE]
                            ?.toString()
                            ?.replace(
                                ",",
                                "",
                    ...
                    listedShares =
                        raw[KrxApiFields.Asset.LISTED_SHARES]
                            ?.toString()
                            ?.replace(
                                ",",
                                "",
                    ...
                    shortBalanceRatio =
                        raw[KrxApiFields.ShortSelling.BALANCE_RATIO]?.toString()?.toDoubleOrNull() ?: 0.0,
                )
            }.sortedBy { it.tradeDate }
