...
import dev.kairoscode.kfc.domain.bond.BondYieldItem
...
internal class KrxBondApiImpl(
    ...
    override suspend fun getBondYieldsByDate(date: LocalDate): BondYieldSnapshot {
        ...
        val outBlock =
            (response["output"] as? List<*>)?.filterIsInstance<Map<String, Any?>>()
                ?: emptyList()

        logger.debug { "Response contains ${outBlock.size} bond records" }

        val yields =
            outBlock.mapNotNull { raw ->
     ...
     */
    private fun String.parseKrxDate(): LocalDate = LocalDate.parse(this, krxDateFormatter)
}
