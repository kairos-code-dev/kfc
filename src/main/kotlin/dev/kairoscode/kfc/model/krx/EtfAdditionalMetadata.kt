...
/**
 * MDCSTAT04703 - ETF 추가 메타데이터
 *
 * ETF 개별종목 종합정보 페이지에서 제공되는 추가 메타데이터 모델입니다.
 *
 ...
 */
data class EtfAdditionalMetadata(
    // 원시 데이터 (유연한 접근용)
    ...
    companion object {
        ...
        /**
         * KRX API 원시 응답으로부터 EtfAdditionalMetadata 생성
         *
         * @param raw KRX API 응답 Map
         * @return EtfAdditionalMetadata 인스턴스
         */
        @Suppress("UNCHECKED_CAST")
        fun fromRaw(raw: Map<*, *>): EtfAdditionalMetadata {
            val rawData = raw.mapKeys { it.key.toString() }
                .mapValues { it.value } as Map<String, Any?>

            return EtfAdditionalMetadata(
                rawData = rawData,
