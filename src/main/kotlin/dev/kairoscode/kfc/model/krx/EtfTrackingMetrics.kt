...
/**
 * MDCSTAT04702 - ETF 추적오차/괴리율 관련 메트릭스
 *
 * ETF 개별종목 종합정보 페이지에서 제공되는 추적오차 및 괴리율 관련 데이터 모델입니다.
 *
...
data class EtfTrackingMetrics(
    ...
    companion object {
        ...
        /**
         * KRX API 원시 응답으로부터 EtfTrackingMetrics 생성
         *
         * @param raw KRX API 응답 Map
         * @return EtfTrackingMetrics 인스턴스
         */
        @Suppress("UNCHECKED_CAST")
        fun fromRaw(raw: Map<*, *>): EtfTrackingMetrics {
            val rawData = raw.mapKeys { it.key.toString() }
                .mapValues { it.value } as Map<String, Any?>

            return EtfTrackingMetrics(
                rawData = rawData,
