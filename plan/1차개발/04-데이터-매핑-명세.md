...
**예제**:
```kotlin
"2.58".toKrxDouble()     // 2.58
"-1.23".toKrxDouble()    // -1.23 (음수 유지)
"0.15".toKrxDouble()     // 0.15
"-".toKrxDouble()        // 0.0
"".toKrxDouble()         // 0.0
```
...
### MDCSTAT04701 - ETF 개별종목 상세정보

**Model Class**: `EtfDetailInfo` (거래일 기준 시간 의존 데이터)
...
### MDCSTAT04702 - ETF 분단위 시세 (분봉 데이터)

**Model Class**: `EtfIntradayBar` (장중 1분 단위 OHLCV)

| KRX 필드 | 한글명 | 원본 타입 | 정규화 타입 | 변환 함수 | 비고 |
|---------|--------|----------|-----------|----------|------|
| TRD_DD | 시간 | String | String | (변환 없음) | HH:MM 형식 |
| TDD_CLSPRC | 현재가 | String | Int | toKrxInt() | |
| TDD_OPNPRC | 시가 | String | Int | toKrxInt() | |
| TDD_HGPRC | 고가 | String | Int | toKrxInt() | |
| TDD_LWPRC | 저가 | String | Int | toKrxInt() | |
| ACC_TRDVOL | 누적거래량 | String | Long | toKrxLong() | |
| BAS_PRC | 기준가 | String | Int | toKrxInt() | |

**특징**:
- 09:00 ~ 14:56까지 1분 간격 데이터 (약 330+ bars)
- 대용량 데이터셋 (SmartRecorder 사용)
- 거래일만 데이터 제공

---

### MDCSTAT04703 - ETF 최근 일별 거래 (10일 요약)

**Model Class**: `EtfRecentDaily` (최근 10거래일 시세 요약)

| KRX 필드 | 한글명 | 원본 타입 | 정규화 타입 | 변환 함수 | 비고 |
|---------|--------|----------|-----------|----------|------|
| TRD_DD | 거래일 | String | LocalDate | toKrxDate() | |
| TDD_CLSPRC | 종가 | String | Int | toKrxInt() | |
| FLUC_TP_CD | 등락구분 | String | Direction | toKrxDirection() | 1=상승, 2=하락, 3=보합 |
| CMPPREVDD_PRC | 전일대비 | String | Int | toKrxInt() | **부호 있음** |
| FLUC_RT | 등락률 | String | Double | toKrxDouble() | **부호 있음** |
| ACC_TRDVOL | 거래량 | String | Long | toKrxLong() | |
| ACC_TRDVAL | 거래대금 | String | Long | toKrxLong() | |
...
**Model Class**: `EtfGeneralInfo` (상장 이후 변경되지 않는 정적 정보)

| KRX 필드 | 한글명 | 원본 타입 | 정규화 타입 | 변환 함수 | 비고 |
|---------|--------|----------|-----------|----------|------|
| ISU_NM | 종목명 | String | String | (변환 없음) | 전체명 |
| ISU_CD | 종목코드 | String | String | (변환 없음) | ISIN 12자리 |
| ISU_SRT_CD | 단축코드 | String | String | (변환 없음) | 6자리 티커 |
| ISU_ABBRV | 종목약명 | String | String | (변환 없음) | |
| ISU_ENG_NM | 영문명 | String | String | (변환 없음) | |
| LIST_DD | 상장일 | String | LocalDate | toKrxDate() | |
| NETASST_TOTAMT | 순자산총액 | String | Long | toKrxLong() | |
| PREVDD_NAV | 전일NAV | String | **BigDecimal** | toKrxBigDecimal() | |
| LIST_SHRS | 상장주식수 | String | Long | toKrxLong() | |
| ETF_TP_CD | ETF유형코드 | String | String | (변환 없음) | |
| SETL_MMDD | 결산월일 | String | String | (변환 없음) | MMDD 형식 |
| EXPD_DD | 만기일 | String | LocalDate? | toKrxDate() | 영구는 null |
| PAR | 액면가 | String | Int | toKrxInt() | |
| ETF_DIVI_DD | 배당기준일 | String | LocalDate? | toKrxDate() | 없으면 null |
| OBJ_STKPRC_IDX | 기초지수 | String | **BigDecimal** | toKrxBigDecimal() | |
| TRACE_YD_MULT | 추적배수 | String | **BigDecimal** | toKrxBigDecimal() | |
| RGT_NETASST_TOTAMT | 권리NAV총액 | String | Long | toKrxLong() | |
| RGT_NAV | 권리NAV | String | **BigDecimal** | toKrxBigDecimal() | |

**특징**:
- 상장 후 거의 변경되지 않는 정적 메타데이터
- 종목명, 상장일, 결산월일, 만기일, 액면가 등 구조적 정보
- MDCSTAT04701(상세정보)과 차별화됨: 04701은 시간 의존 데이터 (가격, NAV, 거래량)

---

### MDCSTAT04705 - PDF 상위 10 종목 (포트폴리오 구성)

**Model Class**: `PortfolioTopItem` (포트폴리오 상위 10개 구성종목)

| KRX 필드 | 한글명 | 원본 타입 | 정규화 타입 | 변환 함수 | 비고 |
|---------|--------|----------|-----------|----------|------|
| COMPST_ISU_CD | 구성종목코드 | String | String | (변환 없음) | 종목코드 또는 혼합 형식 |
...
- 포트폴리오 상위 10개 종목만 조회
- 소규모 데이터셋 (10 records)
- MDCSTAT05001 (전체 포트폴리오)과 달리 상위 10개만 반환

---

### MDCSTAT05001 - PDF (포트폴리오 전체)

**Model Class**: `PortfolioConstituent` (포트폴리오 전체 구성종목)
...
| 필드 | 설명 | 변환 함수 | 타입 |
|------|------|----------|------|
| TDD_CLSPRC (분단위) | 현재가 | toKrxInt() | Int |
| TDD_CLSPRC (일별) | 종가 | toKrxPrice() | BigDecimal |
| ACC_TRDVOL | 거래량 | toKrxLong() | Long |
| MKTCAP | 시가총액 | toKrxAmount() | BigDecimal |
| LIST_SHRS | 상장주식수 | toKrxLong() | Long |
| ASK_TRDVOL | 매도거래량 | toKrxLong() | Long |
| BID_TRDVOL | 매수거래량 | toKrxLong() | Long |

### 음수 가능한 필드 (부호 있음)

| 필드 | 설명 | 변환 함수 | 타입 | 음수 의미 |
|------|------|----------|------|-----------|
| NETBID_TRDVAL | 순매수거래대금 | toKrxSignedLong() | Long | 순매도 |
| NETBID_TRDVOL | 순매수거래량 | toKrxSignedLong() | Long | 순매도 |
| NUM_ITM_VAL21~24 | 투자자별 순매수 | toKrxSignedLong() | Long | 순매도 |
| CMPPREVDD_PRC | 전일대비 | toKrxInt() | Int | 하락 |
| CMPPREVDD_PRC (BigDecimal) | 전일대비 | toKrxPrice() | BigDecimal | 하락 |
| FLUC_RT (Double) | 등락률 | toKrxDouble() | Double | 하락률 |
| FLUC_RT (BigDecimal) | 등락률 | toKrxRate() | BigDecimal | 하락률 |
| DIVRG_RT | 괴리율 | toKrxRate() | BigDecimal | NAV보다 낮음 |
...
| 데이터 종류 | 변환 함수 | 타입 | 스케일 | 예시 |
|------------|----------|------|--------|------|
| 정수 가격 (Int Price) | `toKrxInt()` | Int | - | 분단위 현재가, 시가 |
| 가격 (Price) | `toKrxPrice()` | BigDecimal | 2 | 종가, 시가 |
| 금액 (Amount) | `toKrxAmount()` | BigDecimal | 0 | 시가총액, 거래대금 |
| 부동소수점비율 | `toKrxDouble()` | Double | - | 등락률 (Double) |
| 비율 (Rate) | `toKrxRate()` | BigDecimal | 4 | 등락률 (정밀), 보수율 |
| 고정밀 (NAV/지수) | `toKrxBigDecimal()` | BigDecimal | 원본 | NAV, 지수 |
| 수량 (Quantity) | `toKrxLong()` | Long | - | 거래량, 주식수 |
| 부호있는수량 | `toKrxSignedLong()` | Long | - | 순매수 |
| 날짜 (Date) | `toKrxDate()` | LocalDate | - | 거래일 |
| 방향 (Direction) | `toKrxDirection()` | Direction | - | 등락구분 |
...
- 03-MDCSTAT04701-상세명세.md: EtfDetailInfo 구현 예제
