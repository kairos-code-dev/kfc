...
| ❌ Rate Limiting | 사용 패턴별 차이 | 10 req/sec, 100ms delay |
...
**Q: Rate Limiting은 왜 라이브러리에 없나요?**
- 사용 패턴에 따라 전략이 다름 (배치 vs 실시간)
- 여러 API를 통합 사용 시 제어가 복잡
- 애플리케이션 레벨에서 더 유연하게 관리 가능
...
|------|-------------------------|-------------|
| **역할** | 데이터 수집 | 비즈니스 로직 |
| **예시** | API 호출, 파싱, 타입 변환 | 계산, 조합, 저장, Rate Limiting |
