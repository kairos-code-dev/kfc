...
fun main() =
    runBlocking {
        ...
        println("-".repeat(80))
        val incomeStatement =
            financialsApi.getIncomeStatement(
        ...
        val cashFlowStatement =
            financialsApi.getCashFlowStatement(
                corpCode = samsungCorp.corpCode,
                year = 2023,
                reportType = ReportType.ANNUAL,
                statementType = StatementType.CONSOLIDATED,
        ...
        println("-".repeat(80))
        val allFinancials =
            financialsApi.getAllFinancials(
        ...
        allFinancials.incomeStatement?.let { incomeStmt ->
            println("\n손익계산서:")
            incomeStmt.getRevenue()?.let { println("  매출액: ${String.format("%,d", it.toLong())}원") }
            incomeStmt.getOperatingIncome()?.let { println("  영업이익: ${String.format("%,d", it.toLong())}원") }
            incomeStmt.getNetIncome()?.let { println("  당기순이익: ${String.format("%,d", it.toLong())}원") }
        }
        ...
        companies.forEach { (ticker, name) ->
            ...
            if (corp != null) {
                try {
                    ...
                    val incomeStmt = financials.incomeStatement
                    val balSheet = financials.balanceSheet
